...
export class Node 
{
  ...
  { name: "previous", typeTag: new StructTag(new HexString("0x1"), "option", "Option", [new $.TypeParamIdx(0)]) },
...
export class Tablist 
{
  ...
  static moduleName = moduleName;
  __app: $.AppType | null = null;
  static structName: string = "Tablist";
  static typeParameters: TypeParamDeclType[] = [
    { name: "K", isPhantom: false },
    { name: "V", isPhantom: false }
  ];
  ...
  static TablistParser(data:any, typeTag: TypeTag, repo: AptosParserRepo) : Tablist {
...
export function add_ (
  ...
  let node, old_tail;
  node = new Node({ value: value, previous: $.copy(tablist_ref_mut.tail), next: Stdlib.Option.none_($c, [$p[0]]) }, new SimpleStructTag(Node, [$p[0], $p[1]]));
  Stdlib.Table_with_length.add_(tablist_ref_mut.table, $.copy(key), node, $c, [$p[0], new SimpleStructTag(Node, [$p[0], $p[1]])]);
  if (Stdlib.Option.is_none_(tablist_ref_mut.head, $c, [$p[0]])) {
    ...
    Stdlib.Table_with_length.borrow_mut_(tablist_ref_mut.table, $.copy(old_tail), $c, [$p[0], new SimpleStructTag(Node, [$p[0], $p[1]])]).next = Stdlib.Option.some_($.copy(key), $c, [$p[0]]);
  }
...
export function borrow_ (
  ...
  return Stdlib.Table_with_length.borrow_(tablist_ref.table, $.copy(key), $c, [$p[0], new SimpleStructTag(Node, [$p[0], $p[1]])]).value;
}
...
export function borrow_iterable_ (
  ...
  let node_ref;
  node_ref = Stdlib.Table_with_length.borrow_(tablist_ref.table, $.copy(key), $c, [$p[0], new SimpleStructTag(Node, [$p[0], $p[1]])]);
...
export function borrow_iterable_mut_ (
  ...
  let node_ref_mut;
  node_ref_mut = Stdlib.Table_with_length.borrow_mut_(tablist_ref_mut.table, $.copy(key), $c, [$p[0], new SimpleStructTag(Node, [$p[0], $p[1]])]);
  return [node_ref_mut.value, $.copy(node_ref_mut.previous), $.copy(node_ref_mut.next)];
}
...
export function borrow_mut_ (
  ...
  return Stdlib.Table_with_length.borrow_mut_(tablist_ref_mut.table, $.copy(key), $c, [$p[0], new SimpleStructTag(Node, [$p[0], $p[1]])]).value;
}
...
export function contains_ (
  ...
  return Stdlib.Table_with_length.contains_(tablist_ref.table, $.copy(key), $c, [$p[0], new SimpleStructTag(Node, [$p[0], $p[1]])]);
}
...
export function destroy_empty_ (
  ...
  let { table: table } = tablist;
  Stdlib.Table_with_length.destroy_empty_(table, $c, [$p[0], new SimpleStructTag(Node, [$p[0], $p[1]])]);
  return;
}
...
export function is_empty_ (
  ...
  return Stdlib.Table_with_length.empty_(tablist_ref.table, $c, [$p[0], new SimpleStructTag(Node, [$p[0], $p[1]])]);
}
...
export function length_ (
  ...
  return Stdlib.Table_with_length.length_(tablist_ref.table, $c, [$p[0], new SimpleStructTag(Node, [$p[0], $p[1]])]);
}
...
export function new___ (
  ...
  return new Tablist({ table: Stdlib.Table_with_length.new___($c, [$p[0], new SimpleStructTag(Node, [$p[0], $p[1]])]), head: Stdlib.Option.none_($c, [$p[0]]), tail: Stdlib.Option.none_($c, [$p[0]]) }, new SimpleStructTag(Tablist, [$p[0], $p[1]]));
}
...
export function remove_ (
  ...
  let value;
  [value, , ] = remove_iterable_(tablist_ref_mut, $.copy(key), $c, [$p[0], $p[1]]);
  return value;
}
...
export function remove_iterable_ (
  ...
  let { value: value, previous: previous, next: next } = Stdlib.Table_with_length.remove_(tablist_ref_mut.table, $.copy(key), $c, [$p[0], new SimpleStructTag(Node, [$p[0], $p[1]])]);
  if (Stdlib.Option.is_none_(previous, $c, [$p[0]])) {
    tablist_ref_mut.head = $.copy(next);
  }
  else{
    Stdlib.Table_with_length.borrow_mut_(tablist_ref_mut.table, $.copy(Stdlib.Option.borrow_(previous, $c, [$p[0]])), $c, [$p[0], new SimpleStructTag(Node, [$p[0], $p[1]])]).next = $.copy(next);
  }
  if (Stdlib.Option.is_none_(next, $c, [$p[0]])) {
    tablist_ref_mut.tail = $.copy(previous);
  }
  else{
    Stdlib.Table_with_length.borrow_mut_(tablist_ref_mut.table, $.copy(Stdlib.Option.borrow_(next, $c, [$p[0]])), $c, [$p[0], new SimpleStructTag(Node, [$p[0], $p[1]])]).previous = $.copy(previous);
  }
