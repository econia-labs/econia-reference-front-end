...
export class SignerCapabilityStore 
{
  ...
  static structName: string = "SignerCapabilityStore";
  static typeParameters: TypeParamDeclType[] = [

  ];
  static fields: FieldDeclType[] = [
  { name: "signer_capability", typeTag: new StructTag(new HexString("0x1"), "account", "SignerCapability", []) }];

  signer_capability: Stdlib.Account.SignerCapability;

  constructor(proto: any, public typeTag: TypeTag) {
    this.signer_capability = proto['signer_capability'] as Stdlib.Account.SignerCapability;
  }

  static SignerCapabilityStoreParser(data:any, typeTag: TypeTag, repo: AptosParserRepo) : SignerCapabilityStore {
    const proto = $.parseStructProto(data, typeTag, repo, SignerCapabilityStore);
    return new SignerCapabilityStore(proto, typeTag);
  }

  static async load(repo: AptosParserRepo, client: AptosClient, address: HexString, typeParams: TypeTag[]) {
    const result = await repo.loadResource(client, address, SignerCapabilityStore, typeParams);
    return result as unknown as SignerCapabilityStore;
  }
  static async loadByApp(app: $.AppType, address: HexString, typeParams: TypeTag[]) {
    const result = await app.repo.loadResource(app.client, address, SignerCapabilityStore, typeParams);
    await result.loadFullState(app)
    return result as unknown as SignerCapabilityStore;
  }
  static getTag(): StructTag {
    return new StructTag(moduleAddress, moduleName, "SignerCapabilityStore", []);
  }
...
export function init_module_ (
  ...
  [, signer_capability] = Stdlib.Account.create_resource_account_(econia, $.copy(time_seed), $c);
  $c.move_to(new SimpleStructTag(SignerCapabilityStore), econia, new SignerCapabilityStore({ signer_capability: signer_capability }, new SimpleStructTag(SignerCapabilityStore)));
  return;
}
...
export class App {
  ...
  get SignerCapabilityStore() { return SignerCapabilityStore; }
  async loadSignerCapabilityStore(
    owner: HexString,
    loadFull=true,
    fillCache=true,
  ) {
    const val = await SignerCapabilityStore.load(this.repo, this.client, owner, [] as TypeTag[]);
    if (loadFull) {
