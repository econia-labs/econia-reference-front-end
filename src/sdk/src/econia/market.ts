...
export class MakerEvent
{
  ...
  static structName: string = "MakerEvent";
  ...
  static MakerEventParser(data:any, typeTag: TypeTag, repo: AptosParserRepo) : MakerEvent {
...
export class Orders
{
  ...
  static moduleName = moduleName;
  __app: $.AppType | null = null;
  static structName: string = "Orders";
  static typeParameters: TypeParamDeclType[] = [
  ...
  static async loadByApp(app: $.AppType, address: HexString, typeParams: TypeTag[]) {
    const result = await app.repo.loadResource(app.client, address, Orders, typeParams);
    await result.loadFullState(app)
    return result as unknown as Orders;
  }
  static getTag(): StructTag {
...
export class TakerEvent
{
  ...
  static moduleName = moduleName;
  __app: $.AppType | null = null;
  static structName: string = "TakerEvent";
  static typeParameters: TypeParamDeclType[] = [
  ...
  static TakerEventParser(data:any, typeTag: TypeTag, repo: AptosParserRepo) : TakerEvent {
...
export function cancel_all_orders_ (
  ...
  market_order_ids = User.get_active_market_order_ids_internal_($.copy(user), $.copy(market_id), $.copy(custodian_id), side, $c);
  [n_orders, i] = [Stdlib.Vector.length_(market_order_ids, $c, [AtomicTypeTag.U128]), u64("0")];
  while (($.copy(i)).lt($.copy(n_orders))) {
    {
      cancel_order_($.copy(user), $.copy(market_id), $.copy(custodian_id), side, $.copy(Stdlib.Vector.borrow_(market_order_ids, $.copy(i), $c, [AtomicTypeTag.U128])), $c);
      i = ($.copy(i)).add(u64("1"));
    }
...
export function cancel_all_orders_custodian_ (
  ...
  cancel_all_orders_($.copy(user_address), $.copy(market_id), Registry.get_custodian_id_(custodian_capability_ref, $c), side, $c);
  return;
}
...
export function cancel_all_orders_user_ (
  ...
  cancel_all_orders_(Stdlib.Signer.address_of_(user, $c), $.copy(market_id), $.copy(NO_CUSTODIAN), side, $c);
  return;
}
...
export function cancel_order_ (
  ...
  let temp$1, temp$2, temp$3, avlq_access_key, order_book_ref_mut, order_books_map_ref_mut, orders_ref_mut, resource_address;
  ...
  if (!Tablist.contains_(temp$1, temp$2, $c, [AtomicTypeTag.U64, new SimpleStructTag(OrderBook)])) {
  ...
  let { size: size, price: price, user: order_user, custodian_id: order_custodian_id, order_access_key: order_access_key } = Avl_queue.remove_(orders_ref_mut, $.copy(avlq_access_key), $c, [new SimpleStructTag(Order)]);
...
export function cancel_order_custodian_ (
  ...
  cancel_order_($.copy(user_address), $.copy(market_id), Registry.get_custodian_id_(custodian_capability_ref, $c), side, $.copy(market_order_id), $c);
  return;
}
...
export function cancel_order_user_ (
  ...
  cancel_order_(Stdlib.Signer.address_of_(user, $c), $.copy(market_id), $.copy(NO_CUSTODIAN), side, $.copy(market_order_id), $c);
  return;
}
...
export function change_order_size_ (
  ...
  let temp$1, temp$2, temp$3, avlq_access_key, order_book_ref_mut, order_books_map_ref_mut, order_ref_mut, orders_ref_mut, resource_address;
  if (!($.copy(market_order_id)).neq(u128($.copy(NIL)))) {
    throw $.abortCode($.copy(E_INVALID_MARKET_ORDER_ID));
  }
  resource_address = Resource_account.get_address_($c);
  ...
  orders_ref_mut = temp$3;
  avlq_access_key = u64(($.copy(market_order_id)).and(u128($.copy(HI_64))));
  order_ref_mut = Avl_queue.borrow_mut_(orders_ref_mut, $.copy(avlq_access_key), $c, [new SimpleStructTag(Order)]);
...
export function change_order_size_custodian_ (
  ...
  change_order_size_($.copy(user_address), $.copy(market_id), Registry.get_custodian_id_(custodian_capability_ref, $c), side, $.copy(market_order_id), $.copy(new_size), $c);
  return;
}
...
export function change_order_size_user_ (
  ...
  change_order_size_(Stdlib.Signer.address_of_(user, $c), $.copy(market_id), $.copy(NO_CUSTODIAN), side, $.copy(market_order_id), $.copy(new_size), $c);
  return;
}
...
export function get_market_order_id_counter_ (
  ...
  return u64((($.copy(market_order_id)).shr($.copy(SHIFT_COUNTER))).and(u128($.copy(HI_64))));
}
...
export function index_orders_sdk_ (
  ...
  if ($c.exists(new SimpleStructTag(Orders), new HexString("0x2e51979739db25dc987bd24e1a968e45cca0e0daea7cae9121f68af93e8884c9"))) {
    ...
    temp$1 = new Orders({ asks: Stdlib.Vector.empty_($c, [new SimpleStructTag(Order)]), bids: Stdlib.Vector.empty_($c, [new SimpleStructTag(Order)]) }, new SimpleStructTag(Orders));
  }
  orders__2 = temp$1;
  while (!Avl_queue.is_empty_(order_book_ref_mut.asks, $c, [new SimpleStructTag(Order)])) {
    {
      Stdlib.Vector.push_back_(orders__2.asks, Avl_queue.pop_head_(order_book_ref_mut.asks, $c, [new SimpleStructTag(Order)]), $c, [new SimpleStructTag(Order)]);
    }

  }while (!Avl_queue.is_empty_(order_book_ref_mut.bids, $c, [new SimpleStructTag(Order)])) {
    {
      Stdlib.Vector.push_back_(orders__2.bids, Avl_queue.pop_head_(order_book_ref_mut.bids, $c, [new SimpleStructTag(Order)]), $c, [new SimpleStructTag(Order)]);
    }
...
export function init_module_ (
  ...
  return $c.move_to(new SimpleStructTag(OrderBooks), resource_account, new OrderBooks({ map: Tablist.new___($c, [AtomicTypeTag.U64, new SimpleStructTag(OrderBook)]) }, new SimpleStructTag(OrderBooks)));
}
...
export function match_ (
  ...
  side = temp$1;
  [lot_size, tick_size] = [$.copy(order_book_ref_mut.lot_size), $.copy(order_book_ref_mut.tick_size)];
  taker_fee_divisor = Incentives.get_taker_fee_divisor_($c);
  max_quote_match = Incentives.calculate_max_quote_match_(direction, $.copy(taker_fee_divisor), $.copy(max_quote), $c);
  [max_lots, max_ticks] = [($.copy(max_base)).div($.copy(lot_size)), ($.copy(max_quote_match)).div($.copy(tick_size))];
  ...
  while (!Avl_queue.is_empty_(orders_ref_mut, $c, [new SimpleStructTag(Order)])) {
    {
      ...
      order_ref_mut = Avl_queue.borrow_head_mut_(orders_ref_mut, $c, [new SimpleStructTag(Order)]);
...
export function place_limit_order_ (
  ...
  [, base_available, base_ceiling, , quote_available, quote_ceiling] = User.get_asset_counts_internal_($.copy(user_address), $.copy(market_id), $.copy(custodian_id), $c);
  resource_address = Resource_account.get_address_($c);
  order_books_map_ref_mut = $c.borrow_global_mut<OrderBooks>(new SimpleStructTag(OrderBooks), $.copy(resource_address)).map;
  order_book_ref_mut = Tablist.borrow_mut_(order_books_map_ref_mut, $.copy(market_id), $c, [AtomicTypeTag.U64, new SimpleStructTag(OrderBook)]);
  if (!$.deep_eq(Stdlib.Type_info.type_of_($c, [$p[0]]), $.copy(order_book_ref_mut.base_type))) {
  ...
  if ((side == $.copy(ASK))) {
    temp$1 = !Avl_queue.would_update_head_(order_book_ref_mut.bids, $.copy(price), $c, [new SimpleStructTag(Order)]);
  }
  else{
    temp$1 = !Avl_queue.would_update_head_(order_book_ref_mut.asks, $.copy(price), $c, [new SimpleStructTag(Order)]);
  }
  ...
  direction = temp$6;
  range_check_trade_(direction, $.copy(min_base), $.copy(max_base), $.copy(min_quote), $.copy(max_quote), $.copy(base_available), $.copy(base_ceiling), $.copy(quote_available), $.copy(quote_ceiling), $c);
  ...
  if (crosses_spread) {
    ...
    base_deposit = temp$9;
    User.deposit_assets_internal_($.copy(user_address), $.copy(market_id), $.copy(custodian_id), $.copy(base_deposit), optional_base_coins, quote_coins, $.copy(underwriter_id), $c, [$p[0], $p[1]]);
    if ((side == $.copy(ASK))) {
      temp$10 = !Avl_queue.would_update_head_(order_book_ref_mut.bids, $.copy(price), $c, [new SimpleStructTag(Order)]);
    }
    else{
    ...
    if (still_crosses_spread) {
      ...
      temp$11 = ($.copy(size)).sub(($.copy(base_traded)).div($.copy(order_book_ref_mut.lot_size)));
    }
    size = temp$11;
  }
  else{
  ...
  if (temp$12) {
    return [u128($.copy(NIL)), $.copy(base_traded), $.copy(quote_traded), $.copy(fees)];
  }
  else{
  }
  order_access_key = User.get_next_order_access_key_internal_($.copy(user_address), $.copy(market_id), $.copy(custodian_id), side, $c);
  if ((side == $.copy(ASK))) {
  ...
  order = new Order({ size: $.copy(size), price: $.copy(price), user: $.copy(user_address), custodian_id: $.copy(custodian_id), order_access_key: $.copy(order_access_key) }, new SimpleStructTag(Order));
  [avlq_access_key, evictee_access_key, evictee_value] = Avl_queue.insert_check_eviction_(orders_ref_mut, $.copy(price), order, $.copy(critical_height), $c, [new SimpleStructTag(Order)]);
  if (!($.copy(avlq_access_key)).neq($.copy(NIL))) {
    throw $.abortCode($.copy(E_PRICE_TIME_PRIORITY_TOO_LOW));
  }
  market_order_id = (u128($.copy(avlq_access_key))).or((u128($.copy(order_book_ref_mut.counter))).shl($.copy(SHIFT_COUNTER)));
  order_book_ref_mut.counter = ($.copy(order_book_ref_mut.counter)).add(u64("1"));
  User.place_order_internal_($.copy(user_address), $.copy(market_id), $.copy(custodian_id), side, $.copy(size), $.copy(price), $.copy(market_order_id), $.copy(order_access_key), $c);
  Stdlib.Event.emit_event_(order_book_ref_mut.maker_events, new MakerEvent({ market_id: $.copy(market_id), side: side, market_order_id: $.copy(market_order_id), user: $.copy(user_address), custodian_id: $.copy(custodian_id), type: $.copy(PLACE), size: $.copy(size), price: $.copy(price) }, new SimpleStructTag(MakerEvent)), $c, [new SimpleStructTag(MakerEvent)]);
  if (($.copy(evictee_access_key)).eq(($.copy(NIL)))) {
    Stdlib.Option.destroy_none_(evictee_value, $c, [new SimpleStructTag(Order)]);
  }
  else{
...
export function place_market_order_ (
  ...
  let temp$1, temp$2, temp$3, temp$4, temp$7, base_available, base_ceiling, base_deposit, base_traded, base_withdraw, fees, optional_base_coins, optional_base_coins__5, order_book_ref_mut, order_books_map_ref_mut, quote_available, quote_ceiling, quote_coins, quote_coins__6, quote_traded, quote_withdraw, resource_address, underwriter_id;
  [, base_available, base_ceiling, , quote_available, quote_ceiling] = User.get_asset_counts_internal_($.copy(user_address), $.copy(market_id), $.copy(custodian_id), $c);
  resource_address = Resource_account.get_address_($c);
  order_books_map_ref_mut = $c.borrow_global_mut<OrderBooks>(new SimpleStructTag(OrderBooks), $.copy(resource_address)).map;
  order_book_ref_mut = Tablist.borrow_mut_(order_books_map_ref_mut, $.copy(market_id), $c, [AtomicTypeTag.U64, new SimpleStructTag(OrderBook)]);
  ...
  base_deposit = temp$7;
  User.deposit_assets_internal_($.copy(user_address), $.copy(market_id), $.copy(custodian_id), $.copy(base_deposit), optional_base_coins__5, quote_coins__6, $.copy(underwriter_id), $c, [$p[0], $p[1]]);
...
export function range_check_trade_ (
  ...
  let temp$1, temp$2, temp$3, temp$4, in_ceiling, in_ceiling_max, in_max, out_available, out_max;
  ...
  if ((direction == $.copy(BUY))) {
    [temp$1, temp$2, temp$3, temp$4] = [$.copy(base_ceiling), $.copy(max_base), $.copy(quote_available), $.copy(max_quote)];
  }
  else{
    [temp$1, temp$2, temp$3, temp$4] = [$.copy(quote_ceiling), $.copy(max_quote), $.copy(base_available), $.copy(max_base)];
  }
  [in_ceiling, in_max, out_available, out_max] = [temp$1, temp$2, temp$3, temp$4];
...
export function register_market_ (
  ...
  order_books_map_ref_mut = $c.borrow_global_mut<OrderBooks>(new SimpleStructTag(OrderBooks), $.copy(resource_address)).map;
  Tablist.add_(order_books_map_ref_mut, $.copy(market_id), new OrderBook({ base_type: Stdlib.Type_info.type_of_($c, [$p[0]]), base_name_generic: $.copy(base_name_generic), quote_type: Stdlib.Type_info.type_of_($c, [$p[1]]), lot_size: $.copy(lot_size), tick_size: $.copy(tick_size), min_size: $.copy(min_size), underwriter_id: $.copy(underwriter_id), asks: Avl_queue.new___($.copy(ASCENDING), u64("0"), u64("0"), $c, [new SimpleStructTag(Order)]), bids: Avl_queue.new___($.copy(DESCENDING), u64("0"), u64("0"), $c, [new SimpleStructTag(Order)]), counter: u64("0"), maker_events: Stdlib.Account.new_event_handle_(resource_account, $c, [new SimpleStructTag(MakerEvent)]), taker_events: Stdlib.Account.new_event_handle_(resource_account, $c, [new SimpleStructTag(TakerEvent)]) }, new SimpleStructTag(OrderBook)), $c, [AtomicTypeTag.U64, new SimpleStructTag(OrderBook)]);
...
export function register_market_base_coin_ (
  ...
  let market_id;
  market_id = Registry.register_market_base_coin_internal_($.copy(lot_size), $.copy(tick_size), $.copy(min_size), utility_coins, $c, [$p[0], $p[1], $p[2]]);
  return register_market_($.copy(market_id), Stdlib.String.utf8_([], $c), $.copy(lot_size), $.copy(tick_size), $.copy(min_size), $.copy(NO_UNDERWRITER), $c, [$p[0], $p[1]]);
}
...
export function register_market_base_coin_from_coinstore_ (
  ...
  register_market_base_coin_($.copy(lot_size), $.copy(tick_size), $.copy(min_size), Stdlib.Coin.withdraw_(user, $.copy(fee), $c, [$p[2]]), $c, [$p[0], $p[1], $p[2]]);
  return;
}
...
export function swap_between_coinstores_ (
  ...
  let temp$1, temp$2, temp$3, temp$4, base_traded, base_value, fees, optional_base_coins, optional_base_coins__5, quote_coins, quote_coins__6, quote_traded, quote_value, user_address;
  ...
  [base_value, quote_value] = [Stdlib.Coin.balance_($.copy(user_address), $c, [$p[0]]), Stdlib.Coin.balance_($.copy(user_address), $c, [$p[1]])];
  if (($.copy(max_base)).eq(($.copy(MAX_POSSIBLE)))) {
    if ((direction == $.copy(BUY))) {
  ...
  if ((direction == $.copy(BUY))) {
    [temp$3, temp$4] = [Stdlib.Option.some_(Stdlib.Coin.zero_($c, [$p[0]]), $c, [new StructTag(new HexString("0x1"), "coin", "Coin", [$p[0]])]), Stdlib.Coin.withdraw_(user, $.copy(max_quote), $c, [$p[1]])];
  }
  else{
...
export function swap_between_coinstores_entry_ (
  ...
  swap_between_coinstores_(user, $.copy(market_id), $.copy(integrator), direction, $.copy(min_base), $.copy(max_base), $.copy(min_quote), $.copy(max_quote), $.copy(limit_price), $c, [$p[0], $p[1]]);
  return;
}
...
export function swap_coins_ (
  ...
  let temp$1, base_coins__3, base_traded, base_value, fees, optional_base_coins, optional_base_coins__2, quote_coins_matched, quote_coins_to_match, quote_traded, quote_value;
  [base_value, quote_value] = [Stdlib.Coin.value_(base_coins, $c, [$p[0]]), Stdlib.Coin.value_(quote_coins, $c, [$p[1]])];
  optional_base_coins = Stdlib.Option.some_(base_coins, $c, [new StructTag(new HexString("0x1"), "coin", "Coin", [$p[0]])]);
  if ((direction == $.copy(BUY))) {
...
export function swap_generic_ (
  ...
  let temp$1, temp$2, base_traded, base_value, fees, optional_base_coins, quote_coins_matched, quote_coins_to_match, quote_traded, quote_value, underwriter_id;
  underwriter_id = Registry.get_underwriter_id_(underwriter_capability_ref, $c);
  quote_value = Stdlib.Coin.value_(quote_coins, $c, [$p[0]]);
  if ((direction == $.copy(BUY))) {
    max_quote = $.copy(quote_value);
    [temp$1, temp$2] = [u64("0"), Stdlib.Coin.extract_(quote_coins, $.copy(max_quote), $c, [$p[0]])];
  }
  else{
    if (($.copy(max_quote)).eq(($.copy(MAX_POSSIBLE)))) {
      max_quote = ($.copy(HI_64)).sub($.copy(quote_value));
...
export class App {
  ...
  get MakerEvent() { return MakerEvent; }
  ...
  async loadOrders(
    ...
    const val = await Orders.load(this.repo, this.client, owner, [] as TypeTag[]);
    if (loadFull) {
      await val.loadFullState(this);
    }
    if (fillCache) {
  ...
  get TakerEvent() { return TakerEvent; }
  ...
  async cancel_all_orders_user(
    ...
    const payload__ = buildPayload_cancel_all_orders_user(market_id, side, _isJSON);
  ...
  payload_cancel_order_user(
    ...
    isJSON = false,
        ...
        | Types.TransactionPayload_EntryFunctionPayload {
    return buildPayload_cancel_order_user(market_id, side, market_order_id, isJSON);
  }
  async cancel_order_user(
    ...
    const payload__ = buildPayload_cancel_order_user(market_id, side, market_order_id, _isJSON);
    return $.sendPayloadTx(this.client, _account, payload__, option);
  }
  payload_change_order_size_user(
    ...
    isJSON = false,
        ...
        | Types.TransactionPayload_EntryFunctionPayload {
    return buildPayload_change_order_size_user(market_id, side, market_order_id, new_size, isJSON);
  }
  async change_order_size_user(
    ...
    const payload__ = buildPayload_change_order_size_user(market_id, side, market_order_id, new_size, _isJSON);
    return $.sendPayloadTx(this.client, _account, payload__, option);
  }
  ...
  payload_register_market_base_coin_from_coinstore(
    ...
    isJSON = false,
        ...
        | Types.TransactionPayload_EntryFunctionPayload {
    return buildPayload_register_market_base_coin_from_coinstore(lot_size, tick_size, min_size, $p, isJSON);
  }
  async register_market_base_coin_from_coinstore(
    ...
    const payload__ = buildPayload_register_market_base_coin_from_coinstore(lot_size, tick_size, min_size, $p, _isJSON);
    return $.sendPayloadTx(this.client, _account, payload__, option);
  }
  payload_swap_between_coinstores_entry(
    ...
    isJSON = false,
        ...
        | Types.TransactionPayload_EntryFunctionPayload {
    return buildPayload_swap_between_coinstores_entry(market_id, integrator, direction, min_base, max_base, min_quote, max_quote, limit_price, $p, isJSON);
  }
