...
export class GenericAsset 
{
  ...
  static async load(repo: AptosParserRepo, client: AptosClient, address: HexString, typeParams: TypeTag[]) {
    const result = await repo.loadResource(client, address, GenericAsset, typeParams);
    return result as unknown as GenericAsset;
  }
  static async loadByApp(app: $.AppType, address: HexString, typeParams: TypeTag[]) {
    const result = await app.repo.loadResource(app.client, address, GenericAsset, typeParams);
...
export class MarketRegistrationEvent 
{
  ...
  static moduleName = moduleName;
  __app: $.AppType | null = null;
  static structName: string = "MarketRegistrationEvent";
  static typeParameters: TypeParamDeclType[] = [
  ...
  static MarketRegistrationEventParser(data:any, typeTag: TypeTag, repo: AptosParserRepo) : MarketRegistrationEvent {
    const proto = $.parseStructProto(data, typeTag, repo, MarketRegistrationEvent);
    return new MarketRegistrationEvent(proto, typeTag);
  }

  static getTag(): StructTag {
    return new StructTag(moduleAddress, moduleName, "MarketRegistrationEvent", []);
  }
  async loadFullState(app: $.AppType) {
    await this.base_type.loadFullState(app);
    await this.base_name_generic.loadFullState(app);
...
export class RecognizedMarketEvent 
{
  ...
  static moduleName = moduleName;
  __app: $.AppType | null = null;
  static structName: string = "RecognizedMarketEvent";
  static typeParameters: TypeParamDeclType[] = [
  ...
  constructor(proto: any, public typeTag: TypeTag) {
    ...
    this.recognized_market_info = proto['recognized_market_info'] as Stdlib.Option.Option;
  }

  static RecognizedMarketEventParser(data:any, typeTag: TypeTag, repo: AptosParserRepo) : RecognizedMarketEvent {
    const proto = $.parseStructProto(data, typeTag, repo, RecognizedMarketEvent);
    return new RecognizedMarketEvent(proto, typeTag);
  }

  static getTag(): StructTag {
    return new StructTag(moduleAddress, moduleName, "RecognizedMarketEvent", []);
  }
  async loadFullState(app: $.AppType) {
...
export class RecognizedMarketInfo 
{
  ...
  static moduleName = moduleName;
  __app: $.AppType | null = null;
  static structName: string = "RecognizedMarketInfo";
  static typeParameters: TypeParamDeclType[] = [
  ...
  static RecognizedMarketInfoParser(data:any, typeTag: TypeTag, repo: AptosParserRepo) : RecognizedMarketInfo {
...
export class RecognizedMarkets 
{
  ...
  static moduleName = moduleName;
  __app: $.AppType | null = null;
  static structName: string = "RecognizedMarkets";
  static typeParameters: TypeParamDeclType[] = [
  ...
  constructor(proto: any, public typeTag: TypeTag) {
    ...
    this.recognized_market_events = proto['recognized_market_events'] as Stdlib.Event.EventHandle;
  }

  static RecognizedMarketsParser(data:any, typeTag: TypeTag, repo: AptosParserRepo) : RecognizedMarkets {
    const proto = $.parseStructProto(data, typeTag, repo, RecognizedMarkets);
    return new RecognizedMarkets(proto, typeTag);
  }

  static async load(repo: AptosParserRepo, client: AptosClient, address: HexString, typeParams: TypeTag[]) {
    const result = await repo.loadResource(client, address, RecognizedMarkets, typeParams);
    return result as unknown as RecognizedMarkets;
  }
  static async loadByApp(app: $.AppType, address: HexString, typeParams: TypeTag[]) {
    const result = await app.repo.loadResource(app.client, address, RecognizedMarkets, typeParams);
    await result.loadFullState(app)
    return result as unknown as RecognizedMarkets;
  }
  static getTag(): StructTag {
...
export class Registry 
{
  ...
  constructor(proto: any, public typeTag: TypeTag) {
    ...
    this.market_registration_events = proto['market_registration_events'] as Stdlib.Event.EventHandle;
  }
...
export class UnderwriterCapability 
{
  ...
  static moduleName = moduleName;
  __app: $.AppType | null = null;
  static structName: string = "UnderwriterCapability";
  static typeParameters: TypeParamDeclType[] = [
  ...
  static UnderwriterCapabilityParser(data:any, typeTag: TypeTag, repo: AptosParserRepo) : UnderwriterCapability {
    const proto = $.parseStructProto(data, typeTag, repo, UnderwriterCapability);
    return new UnderwriterCapability(proto, typeTag);
  }

  static getTag(): StructTag {
    return new StructTag(moduleAddress, moduleName, "UnderwriterCapability", []);
  }
  async loadFullState(app: $.AppType) {
    this.__app = app;
  }
...
export function get_market_info_for_market_account_ (
  ...
  return [$.copy(market_info_ref.base_name_generic), $.copy(market_info_ref.lot_size), $.copy(market_info_ref.tick_size), $.copy(market_info_ref.min_size), $.copy(market_info_ref.underwriter_id)];
}
...
export function get_recognized_market_info_ (
  ...
  recognized_market_info_ref = $.copy(Tablist.borrow_(recognized_map_ref, $.copy(trading_pair), $c, [new SimpleStructTag(TradingPair), new SimpleStructTag(RecognizedMarketInfo)]));
  return [$.copy(recognized_market_info_ref.market_id), $.copy(recognized_market_info_ref.lot_size), $.copy(recognized_market_info_ref.tick_size), $.copy(recognized_market_info_ref.min_size), $.copy(recognized_market_info_ref.underwriter_id)];
}
...
export function get_recognized_market_info_base_coin_by_type_ (
  ...
  return get_recognized_market_info_base_coin_(Stdlib.Type_info.type_of_($c, [$p[0]]), Stdlib.Type_info.type_of_($c, [$p[1]]), $c);
}
...
export function get_recognized_market_info_base_generic_ (
  ...
  base_type = Stdlib.Type_info.type_of_($c, [new SimpleStructTag(GenericAsset)]);
  trading_pair = new TradingPair({ base_type: $.copy(base_type), base_name_generic: $.copy(base_name_generic), quote_type: $.copy(quote_type) }, new SimpleStructTag(TradingPair));
  return get_recognized_market_info_($.copy(trading_pair), $c);
}
...
export function get_recognized_market_info_base_generic_by_type_ (
  ...
  return get_recognized_market_info_base_generic_($.copy(base_name_generic), Stdlib.Type_info.type_of_($c, [$p[0]]), $c);
}
...
export function has_recognized_market_base_coin_by_type_ (
  ...
  return has_recognized_market_base_coin_(Stdlib.Type_info.type_of_($c, [$p[0]]), Stdlib.Type_info.type_of_($c, [$p[1]]), $c);
}
...
export function has_recognized_market_base_generic_by_type_ (
  ...
  return has_recognized_market_base_generic_($.copy(base_name_generic), Stdlib.Type_info.type_of_($c, [$p[0]]), $c);
}
...
export function init_module_ (
  ...
  $c.move_to(new SimpleStructTag(Registry), econia, new Registry({ market_id_to_info: Tablist.new___($c, [AtomicTypeTag.U64, new SimpleStructTag(MarketInfo)]), market_info_to_id: Stdlib.Table.new___($c, [new SimpleStructTag(MarketInfo), AtomicTypeTag.U64]), n_custodians: u64("0"), n_underwriters: u64("0"), market_registration_events: Stdlib.Account.new_event_handle_(econia, $c, [new SimpleStructTag(MarketRegistrationEvent)]) }, new SimpleStructTag(Registry)));
  $c.move_to(new SimpleStructTag(RecognizedMarkets), econia, new RecognizedMarkets({ map: Tablist.new___($c, [new SimpleStructTag(TradingPair), new SimpleStructTag(RecognizedMarketInfo)]), recognized_market_events: Stdlib.Account.new_event_handle_(econia, $c, [new SimpleStructTag(RecognizedMarketEvent)]) }, new SimpleStructTag(RecognizedMarkets)));
  return;
}
...
export function register_integrator_fee_store_ (
  ...
  market_info_ref = Tablist.borrow_(market_map_ref, $.copy(market_id), $c, [AtomicTypeTag.U64, new SimpleStructTag(MarketInfo)]);
  if (!$.deep_eq($.copy(market_info_ref.quote_type), Stdlib.Type_info.type_of_($c, [$p[0]]))) {
    throw $.abortCode($.copy(E_INVALID_QUOTE));
  }
  Incentives.register_integrator_fee_store_(integrator, $.copy(market_id), $.copy(tier), utility_coins, $c, [$p[0], $p[1]]);
  return;
}
...
export function register_integrator_fee_store_base_tier_ (
  ...
  register_integrator_fee_store_(integrator, $.copy(market_id), u8("0"), Stdlib.Coin.zero_($c, [$p[1]]), $c, [$p[0], $p[1]]);
  return;
}
...
export function register_integrator_fee_store_from_coinstore_ (
  ...
  register_integrator_fee_store_(integrator, $.copy(market_id), $.copy(tier), Stdlib.Coin.withdraw_(integrator, Incentives.get_tier_activation_fee_($.copy(tier), $c), $c, [$p[1]]), $c, [$p[0], $p[1]]);
  return;
}
...
export function register_market_base_coin_internal_ (
  ...
  return register_market_internal_(Stdlib.Type_info.type_of_($c, [$p[0]]), Stdlib.String.utf8_([], $c), $.copy(lot_size), $.copy(tick_size), $.copy(min_size), $.copy(NO_UNDERWRITER), utility_coins, $c, [$p[1], $p[2]]);
}
...
export function register_market_base_generic_internal_ (
  ...
  return register_market_internal_(Stdlib.Type_info.type_of_($c, [new SimpleStructTag(GenericAsset)]), $.copy(base_name_generic), $.copy(lot_size), $.copy(tick_size), $.copy(min_size), $.copy(underwriter_id), utility_coins, $c, [$p[0], $p[1]]);
}
...
export function register_market_internal_ (
  ...
  let temp$1, temp$2, event_handle_ref_mut, id_to_info_ref_mut, info_to_id_ref_mut, market_id, market_info, quote_type, registry_ref_mut;
  ...
  market_id = (Tablist.length_(id_to_info_ref_mut, $c, [AtomicTypeTag.U64, new SimpleStructTag(MarketInfo)])).add(u64("1"));
  Stdlib.Table.add_(info_to_id_ref_mut, $.copy(market_info), $.copy(market_id), $c, [new SimpleStructTag(MarketInfo), AtomicTypeTag.U64]);
  Tablist.add_(id_to_info_ref_mut, $.copy(market_id), $.copy(market_info), $c, [AtomicTypeTag.U64, new SimpleStructTag(MarketInfo)]);
  event_handle_ref_mut = registry_ref_mut.market_registration_events;
  Stdlib.Event.emit_event_(event_handle_ref_mut, new MarketRegistrationEvent({ market_id: $.copy(market_id), base_type: $.copy(base_type), base_name_generic: $.copy(base_name_generic), quote_type: $.copy(quote_type), lot_size: $.copy(lot_size), tick_size: $.copy(tick_size), min_size: $.copy(min_size), underwriter_id: $.copy(underwriter_id) }, new SimpleStructTag(MarketRegistrationEvent)), $c, [new SimpleStructTag(MarketRegistrationEvent)]);
  Incentives.deposit_market_registration_utility_coins_(utility_coins, $c, [$p[1]]);
...
export function register_underwriter_capability_ (
  ...
  return new UnderwriterCapability({ underwriter_id: $.copy(underwriter_id) }, new SimpleStructTag(UnderwriterCapability));
}
...
export function remove_recognized_market_ (
  ...
  recognized_market_id_for_trading_pair = $.copy(Tablist.borrow_(temp$3, temp$4, $c, [new SimpleStructTag(TradingPair), new SimpleStructTag(RecognizedMarketInfo)]).market_id);
  if (!($.copy(recognized_market_id_for_trading_pair)).eq(($.copy(market_id)))) {
    throw $.abortCode($.copy(E_WRONG_RECOGNIZED_MARKET));
  }
  Tablist.remove_(recognized_map_ref_mut, $.copy(trading_pair), $c, [new SimpleStructTag(TradingPair), new SimpleStructTag(RecognizedMarketInfo)]);
  event_handle_ref_mut = recognized_markets_ref_mut.recognized_market_events;
  Stdlib.Event.emit_event_(event_handle_ref_mut, new RecognizedMarketEvent({ trading_pair: $.copy(trading_pair), recognized_market_info: Stdlib.Option.none_($c, [new SimpleStructTag(RecognizedMarketInfo)]) }, new SimpleStructTag(RecognizedMarketEvent)), $c, [new SimpleStructTag(RecognizedMarketEvent)]);
  return;
}
...
export function remove_recognized_markets_ (
  ...
  while (($.copy(i)).lt($.copy(n_markets))) {
    {
      market_id = $.copy(Stdlib.Vector.borrow_(market_ids, $.copy(i), $c, [AtomicTypeTag.U64]));
...
export function set_recognized_market_ (
  ...
  if (new__) {
    Tablist.add_(recognized_map_ref_mut, $.copy(trading_pair), $.copy(recognized_market_info), $c, [new SimpleStructTag(TradingPair), new SimpleStructTag(RecognizedMarketInfo)]);
  }
  else{
    $.set(Tablist.borrow_mut_(recognized_map_ref_mut, $.copy(trading_pair), $c, [new SimpleStructTag(TradingPair), new SimpleStructTag(RecognizedMarketInfo)]), $.copy(recognized_market_info));
  }
  optional_market_info = Stdlib.Option.some_($.copy(recognized_market_info), $c, [new SimpleStructTag(RecognizedMarketInfo)]);
  event_handle_ref_mut = recognized_markets_ref_mut.recognized_market_events;
  Stdlib.Event.emit_event_(event_handle_ref_mut, new RecognizedMarketEvent({ trading_pair: $.copy(trading_pair), recognized_market_info: $.copy(optional_market_info) }, new SimpleStructTag(RecognizedMarketEvent)), $c, [new SimpleStructTag(RecognizedMarketEvent)]);
  return;
}
...
export class App {
  ...
  async loadGenericAsset(
    owner: HexString,
    loadFull=true,
    fillCache=true,
  ) {
    const val = await GenericAsset.load(this.repo, this.client, owner, [] as TypeTag[]);
    if (loadFull) {
  ...
  get MarketRegistrationEvent() { return MarketRegistrationEvent; }
  get RecognizedMarketEvent() { return RecognizedMarketEvent; }
  get RecognizedMarketInfo() { return RecognizedMarketInfo; }
  get RecognizedMarkets() { return RecognizedMarkets; }
  async loadRecognizedMarkets(
    owner: HexString,
    loadFull=true,
    fillCache=true,
  ) {
    const val = await RecognizedMarkets.load(this.repo, this.client, owner, [] as TypeTag[]);
    if (loadFull) {
      await val.loadFullState(this);
    }
    if (fillCache) {
  ...
  get UnderwriterCapability() { return UnderwriterCapability; }
  payload_register_integrator_fee_store_base_tier(
    ...
    isJSON = false,
        ...
        | Types.TransactionPayload_EntryFunctionPayload {
    return buildPayload_register_integrator_fee_store_base_tier(market_id, $p, isJSON);
  }
  async register_integrator_fee_store_base_tier(
    ...
    const payload__ = buildPayload_register_integrator_fee_store_base_tier(market_id, $p, _isJSON);
  ...
  payload_register_integrator_fee_store_from_coinstore(
    ...
    isJSON = false,
        ...
        | Types.TransactionPayload_EntryFunctionPayload {
    return buildPayload_register_integrator_fee_store_from_coinstore(market_id, tier, $p, isJSON);
  }
  async register_integrator_fee_store_from_coinstore(
    ...
    const payload__ = buildPayload_register_integrator_fee_store_from_coinstore(market_id, tier, $p, _isJSON);
    return $.sendPayloadTx(this.client, _account, payload__, option);
  }
  ...
  async remove_recognized_markets(
    ...
    const payload__ = buildPayload_remove_recognized_markets(market_ids, _isJSON);
    return $.sendPayloadTx(this.client, _account, payload__, option);
  }
  ...
  app_get_recognized_market_info_base_coin(
    ...
    return get_recognized_market_info_base_coin_(base_type, quote_type, this.cache);
  }
  app_get_recognized_market_info_base_coin_by_type(
    $p: TypeTag[],
  ) {
  ...
  app_get_recognized_market_info_base_generic(
    ...
    return get_recognized_market_info_base_generic_(base_name_generic, quote_type, this.cache);
  }
  app_get_recognized_market_info_base_generic_by_type(
      base_name_generic: Stdlib.String.String,
    $p: TypeTag[],
  ) {
    return get_recognized_market_info_base_generic_by_type_(base_name_generic, this.cache, $p);
  }
  app_get_underwriter_id(
      underwriter_capability_ref: UnderwriterCapability,
  ) {
  ...
  app_has_recognized_market_base_coin_by_type(
    $p: TypeTag[],
  ) {
  ...
  app_has_recognized_market_base_generic(
    ...
    return has_recognized_market_base_generic_(base_name_generic, quote_type, this.cache);
  }
  app_has_recognized_market_base_generic_by_type(
      base_name_generic: Stdlib.String.String,
    $p: TypeTag[],
  ) {
    return has_recognized_market_base_generic_by_type_(base_name_generic, this.cache, $p);
  }
}
