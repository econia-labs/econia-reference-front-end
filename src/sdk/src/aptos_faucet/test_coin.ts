...
export class CapStore 
{
  ...
  static structName: string = "CapStore";
  ...
  static CapStoreParser(data:any, typeTag: TypeTag, repo: AptosParserRepo) : CapStore {
    const proto = $.parseStructProto(data, typeTag, repo, CapStore);
    return new CapStore(proto, typeTag);
  }

  static async load(repo: AptosParserRepo, client: AptosClient, address: HexString, typeParams: TypeTag[]) {
    const result = await repo.loadResource(client, address, CapStore, typeParams);
    return result as unknown as CapStore;
  }
  static async loadByApp(app: $.AppType, address: HexString, typeParams: TypeTag[]) {
    const result = await app.repo.loadResource(app.client, address, CapStore, typeParams);
...
export function initialize_ (
  ...
  [burn_cap, freeze_cap, mint_cap] = Stdlib.Coin.initialize_(account, $.copy(name), $.copy(symbol), $.copy(decimals), monitor_supply, $c, [$p[0]]);
  $c.move_to(new SimpleStructTag(CapStore, [$p[0]]), account, new CapStore({ burn_cap: $.copy(burn_cap), freeze_cap: $.copy(freeze_cap), mint_cap: $.copy(mint_cap) }, new SimpleStructTag(CapStore, [$p[0]])));
  return;
}
...
export function mint_ (
  ...
  let cap_store;
  cap_store = $c.borrow_global_mut<CapStore>(new SimpleStructTag(CapStore, [$p[0]]), coin_address_($c, [$p[0]]));
  Stdlib.Coin.deposit_(Stdlib.Signer.address_of_(account, $c), Stdlib.Coin.mint_($.copy(amount), cap_store.mint_cap, $c, [$p[0]]), $c, [$p[0]]);
  return;
}
...
export function loadParsers(repo: AptosParserRepo) {
  repo.addParser("0x7c36a610d1cde8853a692c057e7bd2479ba9d5eeaeceafa24f125c23d2abf942::test_coin::CapStore", CapStore.CapStoreParser);
}
export class App {
  constructor(
